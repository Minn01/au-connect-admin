...
# --ignore-scripts skips the postinstall prisma generate (run in the builder).
...
ARG NEXT_PUBLIC_BASE_PATH=/connect-admin
...
# Build-time placeholders only (never used at runtime). These satisfy the
# requiredEnv() checks in lib/env.ts and prisma generate during `next build`.
